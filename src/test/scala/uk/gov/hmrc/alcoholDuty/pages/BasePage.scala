...
trait BasePage extends Page with Matchers with BrowserDriver with Eventually with WebBrowser {
  ...
  def checkDynamicPageHeader(text: String): Unit =
    text match {
      case "Under-declaration" =>
        driver.findElement(By.xpath("//span[@id='when-did-you-pay-duty-section']")).getText.trim.replaceAll("This section is:\n", "") should equal(
  ...
  def periodKey: String = {
    ...
    firstMonthOfQuarter match {
      ...
      case 4 => s"${yearSuffix}AC" // First month of Q2 -> AA
      case 7 => s"${yearSuffix}AF" // First month of Q3 -> AD
      case 10 => s"${yearSuffix}AI" // First month of Q4 -> AG
    }
